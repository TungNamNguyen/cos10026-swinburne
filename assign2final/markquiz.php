...
<section>
    ...
    <section class="article-section" id="article_section">
        ...
        if (!$conn) {
            ...
            $query1 = "CREATE TABLE IF NOT EXISTS attempts (
            ...
            $query2 = "CREATE TABLE IF NOT EXISTS attempt (
                    uid INT(11),
                    fname VARCHAR(50),
                    lname VARCHAR(50),
                    attempt INT(11) )"; 
            ...
            $query3 = "CREATE TABLE IF NOT EXISTS markquiz (
                    uid INT(11),
                    fname VARCHAR(50),
                    lname VARCHAR(50),
                    q1 VARCHAR(50),
        ...
        mysqli_close($conn);
        if ((isset($_POST["uid"])) or (isset($_POST["uid"])) or (isset($_POST["lname"]))) {
            ...
            $conn = @mysqli_connect($host,$user,$pwd,$sql_db);
            $sql_attempt = "attempt";
            $query = "select * from $sql_attempt where $uid and $fname and $lname;";
            ...
            while ($record = mysqli_fetch_assoc($result)){
                if ($uid == $record["uid"]){
                    ...
                    $attempt++;
                    if ($attempt <= 2){
                        $query = "INSERT INTO $sql_attempt VALUES ('$uid','$fname','$lname','$attempt')";
                        $result = mysqli_query($conn, $query);}
                        mysqli_close($conn);
                        if ($attempt <= 2) {
                                        ...
                                        echo "<p>Something is wrong with $query command </p>";} 
                                            ...
                                            if ($score != 0){
                                                ...
                                                if (!$result){
                                                    ...
                                                    echo "<tr>\n";echo "<td>", $uid, "</td>\n";echo "<td>", $fname, "</td>\n";echo "<td>", $lname, "</td>\n";echo "<td>", $q1, "</td>\n";echo "<td>", $q2, "</td>\n";echo "<td>", $q3, "</td>\n";echo "<td>", $q4, "</td>\n";echo "<td>", $q5, "</td>\n";echo "<td>", $score, "</td>\n";echo "</tr>\n";echo "</table>";}
                                                    mysqli_close($conn);}
                                                    if($attempt == 1 and $q1 != "" or $q2 != "" or $q3 != "" or $q4 != "" or $q5 != ""){ 
                                                        echo "<a href=\"quiz.php\"><button type=\"button\">Return</button></a>";}
                                                    if ($q1 == "" or $q2 == "" or $q3 == "" or $q4 == "" or $q5 == ""){  //error when questions are not filled up
